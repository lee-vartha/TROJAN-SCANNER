...
rule putty_trojan {
    ...
    strings:
        $hex1 = { 3d 22 68 74 74 70 3a 2f 2f 73 63 68 65 6d 61 73 }    // "http://schemas", which suggests a URL that could be C&C
        $hex2 = {43 72 79 40 74 6f 43 61 72 64 20 61 75 74 68 } // cry@toCard auth (suspicious)
        // $hex3 = {22 50 75 54 54 59 22  } // "PuTTY" which is a known trojan name - just a marker
        $upx_sig = { 55 8B EC 51 } // UPX

    condition:
    uint16(0) == 0x5A4D and // confirms status of PE
    ( ($hex1 or $hex2) and // must be at least one suspicious string
        ( $upx_sig ) // plus packer or network call
    ) and
    filesize < (5 * 1024) //small file
}
...
rule arctic_bomb {
    meta:
        description = "Arctic Bomb trojan"
    
    strings:
        $hex1 = { 6b 65 72 6e 65 6c 33 32 2e 64 6c 6c  } // kernel32.dll (LOTL)
